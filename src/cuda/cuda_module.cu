...
void CUDACrossEntropyLoss::forward(bool training) {
    ...
    dim3 block((logitsPerClass-1) / MAX_THREAD_PER_BLOCK + 1, 1, 1);
    ...
    *loss /= count;
    dim3 block2((logits->size-1) / MAX_THREAD_PER_BLOCK + 1, 1, 1);
    dim3 thread_in_block2(MAX_THREAD_PER_BLOCK, 1, 1);
    if (training) {

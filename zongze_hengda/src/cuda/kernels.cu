...
void CrossEntropy_forward(Variable *logits, int *truth, float &total_loss, int &count, int num_classes, bool training) {
    ...
    //int *temp_count = (int *)malloc(truth_size);
    //float *temp_loss = (float *)malloc(loss_size);
    ...
    // accumulate and add count and total_loss variables
    //cudaMemcpy(temp_count, d_count, truth_size, cudaMemcpyDeviceToHost);
    //for (int i=0; i<logits->data.size()/num_classes; i++)	count += temp_count[i];
    //cudaMemcpy(temp_loss, d_loss, loss_size, cudaMemcpyDeviceToHost);
    //for (int i=0; i<logits->data.size()/num_classes; i++)	total_loss += temp_loss[i];
